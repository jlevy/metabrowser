...
class StoreMirror:
    """One published store: the home it lives in, its key, and its identity."""
    ...
    def from_published(cls, published: PublishedSource) -> StoreMirror:
        return cls(home=published.home, store_key=published.store_key, store_id=published.store_id)
    ...
    async def refresh(self) -> RefreshResult:
        update = await update_store(self.home, self.store_key)
