...
does not serve content. :mod:`metabrowser.cache.repository_store` leases a published
store for a pinned commit OID (shared maintenance lock and a durable private ref)
without serving that commit. The CLI acquires that source with ``--no-serve`` or as a
side effect of ``--api /api/cache/…``, and still refuses to serve acquired content.
"""
