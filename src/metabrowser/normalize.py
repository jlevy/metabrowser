...
so revisions and mtimes stay real and only values no fixture controls -- the
sandbox path of the served root, pagination cursors, and wall-clock
measurements -- are rewritten.
...
# fixture can pin them. Unlike mtimes, which fixtures pin with `touch -t`, there
# is no arrangement under which the value is reproducible.
...
def normalize_text(text: str, ctx: NormalizeContext) -> str:
    """Rewrite the served root's sandbox path in free text such as console output."""

    prefix = str(ctx.root)
    # Serving "/" would turn every separator in every string into a placeholder,
    # which mangles the payload instead of stabilizing it.
    if prefix == "/":
        return text
    return re.sub(re.escape(prefix) + _BOUNDARY, ROOT_PLACEHOLDER, text)
