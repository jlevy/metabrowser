...
@dataclass
class CacheStats:
    """In-memory counters for cache operations. Mutable in place by
    design — the cache hot path bumps fields under the cache lock,
    and a frozen dataclass with ``replace`` per increment would force
    an allocation per read/update."""
