...
Focused isolated fixes stay standalone: they can be their own PR of any size when they
will be reviewed and merged on their own.
Once the user declines, drop it and do not raise it again that session.

**Spec-driven work consolidates, then stacks.** Incremental work across a large spec or
bead-driven effort belongs in larger reviewable PRs, then in one stack per major feature
or major phase. A stack should typically be eight PRs or fewer.
Prefer fewer, larger layers over many thin ones.
...
- **Every layer stands alone.** It should build, pass tests, and be independently
  ...
  “Stands alone” also means a reviewer can accept or reject the layer without holding
  the rest of the phase.
  Passing CI is necessary and not sufficient.
...
**Lettered spec headings (`0A`, `0B.1`, `1B-c`) are implementation notes**, not
automatic PR boundaries.
They become layers only when each has its own acceptance boundary.

**Informal `--base` is a stack.** Setting a PR base to another feature branch without
`gh stack` (`init` / `link` / `submit`) is a silent failure.
Declare the stack or flatten onto one branch.

**Fold thin siblings.** Land their commits on the surviving layer, retarget that PR so
the diff *is* the reviewable unit, and close the extras with a pointer.
Do not keep a 1-commit PR “for history.”

Each layer costs a PR, a CI run, and a review cycle.

## Two-Level Review

Review first at the **PR level** (that layer’s diff), then **holistically at the stack
level** (agents, then the human).
A stack that is green PR-by-PR can still be wrong as a sequence: missing foundation, a
thin layer that should have been folded, or more than about eight layers.
The stack-level pass is what catches that.

## Stacks and Beads

A stack and a bead tree are related, but **beads are not PRs**:

- One parent bead for the whole change.
- Child beads are implementation steps.
  Several children may land in one layer.
- Order children with `--depends-on` to match real blockers, not to mint a PR per bead.
- Record the branch name and PR number on the beads that landed in that layer.
- Close a child when its work is in a merged layer, not when every bead in the stack
  merges.
...
Do not merge a stack to the trunk unless the user asked to land it.
