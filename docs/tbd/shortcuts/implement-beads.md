...
5. When the **phase** (not each bead) is a reviewable unit, close any remaining
   completed beads — one bulk call per group that shares a reason
   (`tbd close <id1> <id2> … --reason "..."`), not a per-ID loop — then push and create
   or update **one** PR (`tbd shortcut create-or-update-pr-simple`) and run `tbd sync`.
   Beads are not PRs: several closed beads may share that PR. If the next bead is the
   same spec phase, do not open another PR. Isolated bugfixes that are not part of the
   spec effort stay their own standalone PRs.
   If the branch is based on another feature branch, run `tbd shortcut stacked-prs`
   first; do not hand-set `--base`.
