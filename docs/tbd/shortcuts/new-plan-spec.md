...
3. Begin to fill in the new feature plan doc based on the user’s instructions, stopping
   ...
   - You may break work into a few phases if it helps with incremental testing.
     But **use as few phases as possible.** If it is straightforward, use one phase.
     A spec **phase** is the default PR and stack boundary for large work: one stack per
     major feature or phase, typically eight PRs or fewer.
     Do not add lettered sub-phases (`0B.1`) unless each is an independently reviewable
     acceptance boundary.
     Implementation beads may be finer than phases; PRs may not.
