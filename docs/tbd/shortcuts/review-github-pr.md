...
5. **Perform the code review:**
   ...
   - Review this PR first as its own layer.
     If it is based on another feature branch, check formal stack membership
     (`gh stack view --json` and `gh api "repos/$REPO/stacks?pull_request=$PR_NUMBER"`).
     If the PR is not independently reviewable (thin helper, test, or doc; lettered
     sub-phase; informal `--base` with no formal stack), file a High finding: fold into
     the phase survivor; do not ask for more layers.
   - After the PR-level review, if the PR is in a stack, review the stack holistically
     (order, missing foundation, more than about eight layers).
