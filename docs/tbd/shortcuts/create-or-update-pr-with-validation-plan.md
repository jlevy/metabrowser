...
1. **GitHub CLI setup:**
   ...
   - Resolve the trunk instead of assuming `main`, since repos differ and a wrong base
     ...
     rather than proceeding with an empty base, which would make the create step open
     the PR against nothing.
...
5. **Reviewable-unit test** (do this before creating a PR):

   A PR is a review unit.
   Decide standalone vs spec-driven, then apply the matching rule.
   Fail → do not open a PR; commit on the surviving branch and fold.

   **Standalone work stays standalone.** A focused fix, isolated change, or bugfix can
   be its own PR of any size when it will be reviewed and merged on its own.
   Do not fold it into unrelated spec work, and do not invent a stack for it.

   **Spec or bead-driven incremental work consolidates.** Work that is one slice of a
   large spec or bead-driven effort belongs in a larger reviewable PR, then in a formal
   stack (`tbd shortcut stacked-prs`). Several beads may share one PR.

   Fail the test (for spec-driven work) if any of these is true:
   - The unique diff is one helper, one test file, one doc tweak, or one type or
     contract that only exists to unlock the next PR.
   - A reviewer cannot accept or reject this PR without also reading its parent or
     child.
   - The work is a lettered sub-phase (`0B.1`, `1B-b`) of an unfinished major spec
     phase, and that letter is not itself a named acceptance boundary in the spec.
   - The intended or current base is another feature branch, but `gh stack view --json`
     and the remote `stacks?pull_request=` lookup both say this branch is not in a
     formal stack. That is an informal stack.
     Stop. Run `tbd shortcut stacked-prs`. Do not
     `gh pr create --base <other-feature-branch>` by hand.

   Pass if this is standalone work, or a complete major spec phase, or a focused slice
   with its own acceptance boundary (builds, tests or goldens, independently revertible)
   in a formal stack of typically eight PRs or fewer.

   Beads closed on this branch do not each need a PR.

6. Write a PR title and description with these sections.
...
8. Report the PR URL to the user, summarize the validation plan, and inform them you are
   now waiting for CI.

9. **Wait for CI to pass (CRITICAL):**
...
10. Confirm to the user that CI has passed and the PR is ready for review.
    ...
    publishes a review of the PR, and `tbd shortcut address-pr-review` addresses a
    review the PR receives.
