...
5. **Name the PR boundaries separately from the beads.** Beads are implementation steps.
   PRs are review units: one per major spec phase, then one formal stack per major
   feature or phase (typically eight PRs or fewer).
   Isolated bugfixes stay standalone PRs of any size.
   When you summarize the breakdown, list intended PRs and which child beads land in
   each. Do not imply one PR per bead.
