...
| --- | --- | --- |
| Subject and Git target | `source.py`: `RepositorySubject`, `AttachedFilesystemSubject`, `SourceSession`; `git/process.py`: `GitCommandTarget`, `run_git`, `spawn_git_process`, askpass bridge | Separate session selection from a filesystem path. `GitRevisionSubject` and trusted worktree-free Git targets remain the later revision-source phase |
| Content source | `source.py`: `SourceCapabilities`, `ContentSource`, `ContentHandle`, `FilesystemContentSource`; `inventory_engine/coordinator.py`: `open_subject` | Preserve the filesystem provider while adding capability-aware content sessions without fake filesystem metadata. Directory listing, byte windows, and blob readers remain the later Git-tree phase |
...
| Repository store | `cache/repository_store.py`: `resolve_store`, `stage_fetch`, `publish_refs`, `lease_revision`, `converge_store`, `reclaim_objects`; `cache/records.py`: `StagedFetch`, source aliases, and store state | Own the worktree-free Git database, isolate fetch jobs in private refs, converge proven aliases, publish by generation, and retain leased objects |
