...
| --- | --- | --- |
| Subject and Git target | `source.py`: `RepositorySubject`, `AttachedFilesystemSubject`, `SourceSession`; `git/tree_source.py`: `GitRevisionSubject`; `git/process.py`: `GitCommandTarget`, `run_git`, `spawn_git_process`, askpass bridge | Separate session selection from a filesystem path. Route migration, revision leases, and serving acquired Git remain later phases |
