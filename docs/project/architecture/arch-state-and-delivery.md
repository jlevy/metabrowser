...
lock and a durable `refs/metabrowser/subjects/<oid>` ref; file, raw, tree, container,
and event routes still resolve through the attached folder’s `ContentSource` until a
later slice switches them.
