...
| `source.line-anchors` | interaction | `static/source-line-anchors.js#describe`, `static/source-line-anchors.js#nextFragment`, `static/source-line-anchors.js#keyStep`, `static/source-line-anchors.js#preferredView`, `static/source-line-anchors.js#mount`, `static/source-line-anchors.js#refresh`, `static/plugin-sdk.js#renderSourceView`, `builtin_plugins/markdown/source.js#renderMarkdownSource` | `/api/file` | `node tests/dom/source-line-anchors-session.js` | `cli-ui-source-line-anchors.tryscript.md` |
...
| `document.floating-ui-frame` | paint-exempt | `static/styles.css` | `local-only` | — | Where the `position: fixed` TOC drawer, its toggle, and its backdrop land while the preview pane scrolls is browser layout. The two decisions that place them are pinned in `tests/test_preview_frame_contract.py`: the shell wraps the scrolling `#preview-pane` in a non-scrolling `.kpress-frame`, and only that frame is a containing block for fixed descendants. That the toggle stays in view at any scroll depth, in a trusted folder and a served mirror, is step 5 of section 5.9 of the v0.12 QA runbook |
