...
| `/api/tree` | Navigation subtrees. `types` and `min_size` work for every source that supplies them; Git tree listings have no ls-tree sizes, so `min_size` returns `unsupported_for_subject`. `recency` and `include_ignored` require those declared source capabilities and otherwise return `unsupported_for_subject`. On a Git pin the payload keeps Git-native `entries` and also projects a SPA `tree` array (`dir`/`file`/`symlink`, `GitPath` wires, no mtime/size/ignore); gitlinks are files, not directories. File nodes include `logical_ext` from the display suffix. Omitted size and mtime leave tally chrome empty rather than pending |
