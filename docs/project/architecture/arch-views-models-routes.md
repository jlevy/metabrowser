...
| `/raw` | Bounded raw bytes through the active source’s content reader; oversized content is refused before an unbounded object read. A Git subject reads blobs by `GitPath` and follows in-tree relative symlink blobs. LFS pointers are stored pointer bytes; a missing blob, including a promisor miss, is 404 |
