...
| `/api/catalog` | One-shot Quick File universe. A `GitRevisionSubject` lists recursive blob names (`p` GitPath wire, `e` display suffix, `n` display basename) and is already complete; a truncated index is an empty truncated snapshot. Index progress, capabilities, and the event stream still refuse a Git pin |
