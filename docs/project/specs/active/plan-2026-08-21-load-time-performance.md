...
| H58 | Main-thread work during load is bounded by the data, not by a budget. The UI thread should never be blocked by backend work: responsiveness independent of server latency, bandwidth, and tree size, tuned for a local server but not *depending* on one, so `--remote` over a tunnel shows content later rather than freezing. The invariant is constant-bounded synchronous slices however much data arrives — coalesce events into one render of the latest state per frame, deadline-slice any inventory-sized loop, aggregate in a worker, and let speculation yield to interaction. The server learned this in exp-005 and exp-007; the client has not. | `long_task_max_ms` and `interaction_max_ms` between navigation and settle, from `probe-boot.js` attached at navigation, **measured visible**; target no task over 200 ms | Open, **P1** (`mb-y2ft`). Attribution instrumented and the first suspect REFUTED: on a 241,063-file tree the live-update path sums to 137 ms (`fileStoreApplySnapshot` 48 ms max, `fileStoreApplyChange` 8 ms max over 45 calls, `applyTreeFilters` 0) against a reported 71 s of blocking — so whatever burns the time is not the event path, and no fix should be sized until a profile names it. Every long-task figure recorded so far is **void**: they were taken through a hidden pane, and Chromium throttles hidden tabs into manufacturing exactly this signal. The DOM is small (5,124 nodes, 315 rows), so it is not layout volume either. Next step is one visible-tab baseline, then the JS Self-Profiling API (needs a dev-only `Document-Policy: js-profiling` header) to name the function |
