...
| H58 | Main-thread work during load is bounded by the data, not by a budget: on a real 241,063-file tree the page spends 53.9% of its 117 s load with the thread blocked (71.3 s in 59 long tasks, 21 over a second, worst 5,967 ms), against 0.6.0’s 31.2% of 442 s — better in every absolute, denser while it lasts, and densest in the first seconds, when discovery is fastest and the reader is forming their first impression. Transfer volume is not the cause: 598 KB across 181 requests. The correct invariant is constant-bounded synchronous slices however much data arrives — coalesce events into per-frame renders of the latest model, deadline-slice any inventory-sized loop, move tally and rollup aggregation into a worker, and let idle-time speculation yield to interaction. The server learned this in exp-005/exp-007; the client has not. | `long_task_ms_total` and blocked share between navigation and settle, from the longtask observer installed at load (record the install time: a share without its window is meaningless); target no task over 200 ms and blocked share under 10% | Open, **P1** (`mb-y2ft`). The measurement instrument and its two calibration traps — synthetic `.click()` never reaches the handler, `offsetParent` misreads `visibility: hidden` — are recorded in the bead. What the long tasks actually run is unattributed; a Performance-panel profile names the function in one run, and the fix should not be sized before that |
