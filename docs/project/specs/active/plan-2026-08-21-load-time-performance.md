...
| H53 | The tree region is painted twice per load — inlined rows, then fetched rows — each a wholesale replacement of the whole panel. | Repaints per region per load, target 1 | Open (`mb-t5x3`). exp-009 added the metric; exp-010 confirmed it is a regression rather than a standing cost, measuring 1 paint before the campaign against 2 after. The figure was 3 until the metric was fixed to count only `renderTreeNodes:root`: it had counted every render span, and `:inline` is an outer span around a call that emits its own `:root`, so one paint was charged twice. H11 is the fix. Should extend past the tree region to the preview pane and the filter bar |
