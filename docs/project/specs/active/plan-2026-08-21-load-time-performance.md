...
| H65 | The walker stamps a write token on every entry with `dataclasses.replace`, which reads all twenty `FsEntry` fields back through string-keyed `getattr` and reruns the generated `__init__` over them. Building the copy positionally is the same constructor without the reflection. | `walk_to_settled_ms` on a 60,000-file corpus, measured in process | **Confirmed and fixed** ([exp-022](../../../../explorations/performance-loop/experiments/exp-022-the-walker-builds-each-entry-once.md)). 3.5 us to 1.3 us per copy; 1,847 ms to 1,653 on disjoint ranges |
| H66 | Leaf, file, and tracked-file aggregates hang off the same ancestor chain and the ordinary walker event moves all three by the same parent, so adjusting them separately climbs that chain three times. Netting the deltas per parent and climbing once removes two traversals in three. | `walk_to_settled_ms`, same corpus | **Rejected** (exp-022). 1,645 ms against 1,653 on overlapping ranges. The traversals it saved were paid back by the dict and closure allocated per entry to net them in. Right about the redundancy, wrong about where the time was |
| H67 | The walker’s add path — no existing entry, which is every entry of a first walk — has exactly one parent, so it needs no delta map at all. | `walk_to_settled_ms`, same corpus | **Confirmed and fixed** (exp-022). 1,645 ms to 1,277 on disjoint ranges, which is H66’s idea with the allocation removed from the path that never needed it |
| H68 | Every file in a directory climbs the same ancestor chain, so a 60,000-file tree re-derives a few thousand distinct chains once per file. Memoizing them removes roughly 240,000 `rsplit` calls. | `walk_to_settled_ms`, same corpus | **Rejected** ([exp-023](../../../../explorations/performance-loop/experiments/exp-023-the-instrument-was-measuring-the-order-it-ran-in.md)). 1,247 ms against 1,203, nominally slower on overlapping ranges. Shape 2 is four components deep, so `str.rsplit` at C speed costs about what a dict lookup and tuple iteration cost. Reverted |
| H69 | `require_canonical_inventory_path` runs 248,826 times per walk and its `split("/")` allocates a list each time; substring scans decide the same thing without allocating. | Per-call cost, then `walk_to_settled_ms` | **Refused before implementation** (exp-023). 1.05x on a deep path and 0.72x on a short one, because a short path costs less to split than to scan six times |
| H70 | `derive_ext` normalizes separators and splits a path out of what the walker passes as a bare filename, allocating twice per call before it starts. | Per-call cost, then `walk_to_settled_ms` | **Rejected** (exp-023). 1.11x, about 3 ms over 60,000 calls |
| H71 | `for_observed_file` imports `derive_ext` inside the function on every call and `_internal_entry` binds twenty keywords; a module-level import and positional construction remove both. | Per-call cost, then `walk_to_settled_ms` | **Confirmed in isolation, not separable at walk scale** (exp-023). 34 ms over 62,210 entries, against a spread of roughly 50 ms. Kept: one half deletes a function-level import |
