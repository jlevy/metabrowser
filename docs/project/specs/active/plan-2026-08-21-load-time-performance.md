...
| # | Next | Why here |
| --- | --- | --- |
| 1 | **H30** — the gitignore build (`mb-iyat`) | 19-23 s before the first row can exist, larger than the walk it precedes. The single largest fixed cost measured anywhere in this plan, and nothing else can help while it runs |
| 2 | **H27** — rows respond without the tally pass (`mb-74ad`) | The first lever on H31’s loop: it is the per-request cost that steals from the walker. Nearly a payload-shape change, since `updateFilterTallies` already guards every field |
| 3 | **H31** — the contention loop itself (`mb-jalf`) | After H27, re-measure attached against unattached. What remains decides whether serving CPU needs an explicit bound during a scan, or the walk needs to leave the serving process entirely |
