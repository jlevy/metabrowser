...
| H16 | Time to first row on a real repository does not behave like time to first row on `build_corpus`, whose 972 directories are uniformly wide and whose files are stubs. | Every metric, against a checked-out repository of comparable size | Open, **P0, next round** (`mb-6t3n`). Every browser number so far comes from a corpus whose root has thirteen children and whose every file is gitignored by the repository above it. It is the validity multiplier for H4, H7, and the 200-row cap in exp-004 — cheap to build, and it protects every round after it |
