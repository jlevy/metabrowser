...
| `src/metabrowser/cache/acquire.py` | `acquire_file_source`, `acquire_into_staging`, `publish_from_staging` | Acquire and publish a validated worktree-free store, then create the source alias as the final atomic visibility commit. A published `file://` hit is reused without `open_cache` |
