"""Tests for `metabrowser remote` — SSH command construction."""
...
class TestBuildSshTunnelCommand:
    ...
    def test_plain_ssh_basic(self) -> None:
        cmd = build_ssh_tunnel_command(
            "user@myhost",
            remote_cmd="metabrowser serve /mnt/filestore/runs --port 8411 --host 127.0.0.1 --no-open",
            local_port=8411,
            remote_port=8411,
        )
        assert cmd[0] == "ssh"
        ...
        assert "metabrowser" in cmd[-1]

    def test_plain_ssh_with_options(self) -> None:
        cmd = build_ssh_tunnel_command(
            "devbox",
            remote_cmd="metabrowser serve /data/runs --port 8500 --host 127.0.0.1 --no-open",
            local_port=9000,
            remote_port=8500,
    ...
    def test_plain_ssh_different_ports(self) -> None:
        cmd = build_ssh_tunnel_command(
            "user@host",
            remote_cmd="metabrowser serve /data --port 8080 --host 127.0.0.1 --no-open",
            local_port=9090,
            remote_port=8080,
        )
        tunnel_idx = cmd.index("-L")
    ...
    def test_gcp_mode_no_project(self) -> None:
        cmd = build_ssh_tunnel_command(
            "my-vm",
            remote_cmd="metabrowser serve /data/runs --port 8411 --host 127.0.0.1 --no-open",
            local_port=8411,
            remote_port=8411,
            gcp=True,
            zone="us-east1-b",
...
class TestStdinWatchdog:
    ...
    def test_wrapper_structure(self) -> None:
        wrapped = wrap_with_stdin_watchdog("metabrowser serve /data/runs --port 8411")
    ...
    def test_wrapper_preserves_inner_command(self) -> None:
        inner = 'export PATH="$HOME/.local/bin:$PATH" && metabrowser serve /data --port 8411'
