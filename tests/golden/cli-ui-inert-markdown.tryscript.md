...
with no class, `name`, `data-*`, style, or event attribute, no `id` the document wrote,
and no SVG, MathML, media, stylesheet, frame, or form.
...
pin, and turns outside images into links.
Its headings get the anchors github.com gives them (`user-content-` and the slug of the
heading’s text, as `hostile-readme`), and its fragment links reach them (`#readme`
becomes `#user-content-readme`). A pull-request comment makes every reference absolute
against the pull request’s github.com page and every image a link, and has no anchors.
