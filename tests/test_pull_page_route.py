...
def test_the_page_parses_no_html_but_sanitized_markdown() -> None:
    ...
    The one ``innerHTML`` write is KPress's Markdown, sanitized on the server, and links in
    it pass through ``safeLink``; every outside link opens with no opener or referrer.
    """

    source = PAGE_JS.read_text(encoding="utf-8")
    assert source.count("innerHTML") == 1
    assert "holder.innerHTML = String(rendered.html);" in source
    assert "insertAdjacentHTML" not in source and "outerHTML" not in source
    assert source.count('rel: "noopener noreferrer"') == 1
    assert 'link.setAttribute("rel", "noopener noreferrer");' in source
