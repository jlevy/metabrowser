...
//
// Same shape as markdown-mount-behavior.js: import the renderer module, drive
...
// bytes-view.js imports a sibling module, which a data-URL import cannot
// resolve. Link real files through vm modules, the same way load-plugins.js
...
(async () => {
  const { DEFAULT_ACCENT_RUN_BUDGET } = await importModule(
    "src/metabrowser/builtin_plugins/binary/byte-format.js",
  );
  const { mountBytesView, renderChunkState, decodeBase64, measureSurface } = await importModule(
    "src/metabrowser/builtin_plugins/binary/bytes-view.js",
  );
